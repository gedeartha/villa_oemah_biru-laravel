<x-layouts.base>
    <div>
        <div>
            <div class="font-bold text-xl text-primary tracking-wider mb-6">Tambah Fasilitas Extra</div>
        </div>

        <div class="grid grid-cols-12 gap-x-4">
            <div class="col-span-7">
                <x-card>
                    <form action="{{ $addons->id }}/update" method="POST" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="grid grid-cols-12 gap-y-4 p-6">

                            <div class="col-span-4 my-auto">
                                <div class="font-semibold text-sm text-primary">Nama Fasilitas Extra</div>
                            </div>
                            <div class="col-span-8">
                                <input type="text" id="name" name="name"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                    value="{{ $addons->name }}" required>
                            </div>
                            ...
                            <div class="col-span-8 flex space-x-3 items-center">
                                ...
                                <label for="status" class="inline-flex relative items-center mr-5 cursor-pointer">
                                    <input type="checkbox" name="status" id="status" class="sr-only peer"
                                        @checked(old('Aktif', $addons->status == 'Aktif'))>
                                    <div
                    ...
                    <div class="px-6 -mt-16 pb-6">
                        <form action="{{ $addons->id }}/delete" method="POST" enctype="multipart/form-data">
                            @method('DELETE')
                            @csrf
